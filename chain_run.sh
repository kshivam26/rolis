...
num_threads=(1 2 4 6 8 10)         # Replace these values with your actual num_threads values
crpc_options=(4 1 0)         # Replace these values with your actual crpc_options values
bw_options=(5 3 1)
MAX_RETRIES=3
      ...
      for option_value in "${crpc_options[@]}"; do        
        ...
        for ((i = 1; i <= 3; i++)); do
          ...
          while [[ -z $throughput_value ]]; do
            ...
            timeout 4m sudo docker exec -i chain_f2 /root/rolis/cpuset_b2.sh "$thread_value" "$option_value" >& crpc_evaluation/f2.txt&
            pid3=$!
            echo "$pid3"
          ...
          evaluation_file="crpc_evaluation/evaluation_${bw_option}_gbps.txt"
